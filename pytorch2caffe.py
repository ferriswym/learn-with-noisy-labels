import sys
sys.path.append('/home/yuming/projects/caffe')
...
layer_dict = {'MkldnnConvolutionBackward': 'Convolution',
...
#    save_prototxt(net_info, protofile)
    ...
    def convert_layer(func):
        if True:
            ...
            if parent_type == 'MkldnnConvolutionBackward':
...
def pytorch2prototxt(input_var, output_var):
    ...
    def add_layer(func):
        ...
        layer['name'] = layer['type'] + str(layer_id)
        ...
        elif parent_type == 'MkldnnConvolutionBackward':
            # Only for UpsamplingCaffe
#            if func.transposed is True and func.next_functions[1][0] is None:
            if hasattr(func, 'transposed') and func.next_functions[1][0] is None:
        ...
        elif parent_type == 'ThresholdBackward1':
            parent_top = parent_bottoms[0]
        elif parent_type == 'MaxPool2DWithIndicesBackward':
...
if __name__ == '__main__':
    ...
    state_dict = torch.load('/home/yuming/projects/learn-with-noisy-labels/models/model_last.pth')
...
#    input_var = Variable(torch.rand(1, 3, 299, 299))
    ...
    # plot graph to png
    output_dir = 'results'
#    plot_graph(output_var, os.path.join(output_dir, 'squeezenet.dot'))

    pytorch2caffe(input_var, output_var, os.path.join(output_dir, 'deploy.prototxt'),
                  os.path.join(output_dir, 'pytorch-.caffemodel'))
